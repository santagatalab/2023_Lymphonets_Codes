function Step4_CycIF_measurements_nucdilation1pix(filename, options)  
...
for k = 1:length(filename.folders.fols)
    ...
    try
        ...
        disp(coordmat)
        disp('Coordinate file not found')
        continue
    end
    ...
    for i1 = 1:r
        for i2 = 1:c
            ...
            segfile = [filename.folders.main filename.folders.output filename.folders.segmimages FieldFile '_Seg.tif'];
            cyt_segfile = [filename.folders.main filename.folders.output filename.folders.segmimages FieldFile '_NucCytSeg.tif'];
            ...
            % dilate slightly the nuc image
            lb_NucMask= imdilate(lb_NucMask,offsetstrel('ball',1,0));
            ...
            Results.CentroidX(startcell:endcell,1) = CentroidMat(1,:)+Coordinates.Field{i1,i2}(2);
            Results.CentroidY(startcell:endcell,1) = CentroidMat(2,:)+Coordinates.Field{i1,i2}(4);
